...
The list is filtered and updated through the `ListObserver#updateCurrentFilteredList` method in the `Logic` component so that it works across all lists (master list, blacklist, whitelist and overdue list). Since the person list is of a `FilteredList<>` type, filtering of the list can be handled by Java's `FilteredList` class method called `setPredicate`.
...
**Alternative 1 (current choice):** These person to apply the commands on are determined in the constructors. +
